...
# Create the g3log static library
   SET(G3LOG_STATIC_LIB g3logger_static)
   ADD_LIBRARY(${G3LOG_STATIC_LIB} ${SRC_FILES})
   SET_TARGET_PROPERTIES(${G3LOG_STATIC_LIB} PROPERTIES
      LINKER_LANGUAGE CXX
      OUTPUT_NAME g3logger_static
      CLEAN_DIRECT_OUTPUT 1)
   TARGET_LINK_LIBRARIES(${G3LOG_STATIC_LIB} ${PLATFORM_LINK_LIBRIES})
   TARGET_INCLUDE_DIRECTORIES(${G3LOG_STATIC_LIB} PUBLIC ${LOG_SRC})
   
   # Set current library for build examples and tests
   # use default static, because the shared can not build below on this script
   SET(G3LOG_LIBRARY ${G3LOG_STATIC_LIB})
   
# Create the g3log shared library
IF( NOT(MSVC OR MINGW) OR 
    (NOT ${ADD_BUILD_WIN_SHARED}.x STREQUAL ".x" AND MSVC AND NOT(CMAKE_VERSION LESS 3.4))
  )
   
   SET(G3LOG_SHARED_LIB g3logger)

   # set CMAKE_WINDOWS_EXPORT_ALL_SYMBOLS before add_library
   ...
   ADD_LIBRARY(${G3LOG_SHARED_LIB} SHARED ${SRC_FILES})
   SET(${G3LOG_SHARED_LIB}_VERSION_STRING ${VERSION})
   MESSAGE("Creating ${G3LOG_SHARED_LIB} VERSION: " ${VERSION})
   SET_TARGET_PROPERTIES(g3logger PROPERTIES LINKER_LANGUAGE CXX SOVERSION ${VERSION})

   SET_TARGET_PROPERTIES(${G3LOG_SHARED_LIB} PROPERTIES
      LINKER_LANGUAGE CXX
      OUTPUT_NAME g3logger
      CLEAN_DIRECT_OUTPUT 1)

   IF(APPLE)
      SET_TARGET_PROPERTIES(${G3LOG_SHARED_LIB} PROPERTIES MACOSX_RPATH TRUE)
   ENDIF()

   TARGET_LINK_LIBRARIES(${G3LOG_SHARED_LIB} ${PLATFORM_LINK_LIBRIES})

   # Kjell: This is likely not necessary, except for Windows?
   TARGET_INCLUDE_DIRECTORIES(${G3LOG_SHARED_LIB} PUBLIC ${LOG_SRC})
   
   # Set current library for build examples and tests
   SET(G3LOG_LIBRARY ${G3LOG_SHARED_LIB})
ENDIF()
